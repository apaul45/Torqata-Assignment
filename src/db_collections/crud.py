...
router = APIRouter(tags=["crud-routes"])
...
async def update_show(
    show_id: str, show: UpdateShowModel = Body(...), user=Depends(get_current_user)
):
    show = {
        k: v for k, v in show.dict().items() if v is not None
    }  # Needed to remove missing fields
    if len(show) > 0:
        updated_show = await imdb_collection.update_one(
            {"show_id": show_id}, {"$set": show}
        )
